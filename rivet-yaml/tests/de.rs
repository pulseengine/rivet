...
//! Upstream `serde_yaml` 0.9.34 `tests/test_de.rs`, ported (REQ-346).
//!
//! This is the specification rivet's own parser has to meet when it replaces
//! the serde_yaml delegate behind `rivet_yaml::from_str` (step 3). Today every
//! test here passes through that delegate; the point of porting them first is
//! that step 3 cannot quietly change any of these behaviours.
//!
//! Changes from upstream, and nothing else:
//! - `serde_yaml::` paths are `rivet_yaml::`; `indoc!` literals are written
//!   out already dedented; derives come from `serde`.
//! - The harness drops upstream's `Deserializer::from_str` leg. rivet-yaml has
//!   no streaming multi-document `Deserializer`: rivet reads one document per
//!   file. `test_stateful` therefore drives its seed through a parsed `Value`
//!   (which is itself a `Deserializer`) instead of the streaming one.
//! - `rivet_yaml::from_str` requires `DeserializeOwned`, so the harness bounds
//!   are `DeserializeOwned` and upstream's `test_borrowed` (whose whole point
//!   is `Vec<&str>` borrowed from the input) is NOT ported. That narrowing is
//!   deliberate: a CST-backed deserializer does not lend out input borrows,
//!   and rivet compiles without ever asking for one.
//!
//! Two groups. `within_rivet_subset` must keep passing unchanged through
//! step 3. `outside_rivet_subset` uses anchors, aliases or tags, which rivet's
//! parser refuses by design — no alias expansion means no YAML bomb (SLS-5).
//! Those pass today only because parsing is delegated. When step 3 turns them
//! red, each becomes a test that the construct is REFUSED with a message
//! naming the limitation; that is a recorded change of behaviour, not a
//! re-baseline, and it has to be written into REQ-346 when it happens.
    ...
    T: serde::de::DeserializeOwned + PartialEq + Debug,
{
    ...
    rivet_yaml::from_str::<Value>(yaml).unwrap();
    rivet_yaml::from_str::<serde::de::IgnoredAny>(yaml).unwrap();
}

mod within_rivet_subset {
    use super::*;
    ...
    fn test_i128_big() {
        ...
        let yaml = "-9223372036854775809\n";
        assert_eq!(expected, rivet_yaml::from_str::<i128>(yaml).unwrap());

        let octal = "-0o1000000000000000000001\n";
        assert_eq!(expected, rivet_yaml::from_str::<i128>(octal).unwrap());
    }
    ...
    fn test_u128_big() {
        ...
        assert_eq!(expected, rivet_yaml::from_str::<u128>(yaml).unwrap());

        let octal = "0o2000000000000000000000\n";
        assert_eq!(expected, rivet_yaml::from_str::<u128>(octal).unwrap());
    }
    ...
    fn test_de_mapping() {
        ...
        struct Data {
            pub substructure: rivet_yaml::Mapping,
        }
        let yaml = "substructure:\n  a: 'foo'\n  b: 'bar'\n";

        let mut expected = Data {
            substructure: rivet_yaml::Mapping::new(),
        };
        expected.substructure.insert(
            rivet_yaml::Value::String("a".to_owned()),
            rivet_yaml::Value::String("foo".to_owned()),
        );
        expected.substructure.insert(
            rivet_yaml::Value::String("b".to_owned()),
            rivet_yaml::Value::String("bar".to_owned()),
        );
    ...
    fn test_numbers() {
        ...
        for &(yaml, expected) in &cases {
            let value = rivet_yaml::from_str::<Value>(yaml).unwrap();
            match value {
    ...
    fn test_nan() {
        ...
        assert!(
            rivet_yaml::from_str::<f32>(".nan")
                .unwrap()
                .is_sign_positive()
        );
        assert!(
            rivet_yaml::from_str::<f64>(".nan")
                .unwrap()
                .is_sign_positive()
        );
    }
    ...
    fn test_stateful() {
        ...
        for &(yaml, seed, expected) in &cases {
            // Upstream drives the seed through the streaming Deserializer; the
            // parsed Value is rivet-yaml's Deserializer (see module docs).
            let value: Value = rivet_yaml::from_str(yaml).unwrap();
            let deserialized = serde::de::DeserializeSeed::deserialize(Seed(seed), value).unwrap();
            assert_eq!(expected, deserialized);
    ...
    fn test_no_required_fields() {
        ...
        for document in ["", "# comment\n"] {
            ...
            let deserialized: NoRequiredFields = rivet_yaml::from_str(document).unwrap();
            assert_eq!(expected, deserialized);

            let expected = Vec::<String>::new();
            let deserialized: Vec<String> = rivet_yaml::from_str(document).unwrap();
            assert_eq!(expected, deserialized);

            let expected = BTreeMap::new();
            let deserialized: BTreeMap<char, usize> = rivet_yaml::from_str(document).unwrap();
            assert_eq!(expected, deserialized);

            let expected = None;
            let deserialized: Option<String> = rivet_yaml::from_str(document).unwrap();
            assert_eq!(expected, deserialized);

            let expected = Value::Null;
            let deserialized: Value = rivet_yaml::from_str(document).unwrap();
            assert_eq!(expected, deserialized);
        }
    }
    ...
    fn test_empty_scalar() {
        ...
        let yaml = "thing:\n";
        let expected = Struct {
            thing: rivet_yaml::Sequence::new(),
        };
        test_de(yaml, &expected);

        let expected = Struct {
            thing: rivet_yaml::Mapping::new(),
        };
        test_de(yaml, &expected);
    }
...
/// Anchors, aliases and tags: refused by rivet's parser by design (see the
/// module docs). Passing today only because parsing is delegated.
mod outside_rivet_subset {
    use super::*;
...
";
        ...
        assert_eq!(expected, rivet_yaml::from_str::<Data>(yaml).unwrap());
    }
