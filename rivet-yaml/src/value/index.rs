...
/// A type that can be used to index into a `rivet_yaml::Value`. See the `get`
    ...
    I: Index,
{
    ...
    /// Index into a `rivet_yaml::Value` using the syntax `value[0]` or
    /// `value["k"]`.
    ///
    ...
    /// let data: rivet_yaml::Value = rivet_yaml::from_str(r#"{ x: { y: [z, zz] } }"#)?;
    ///
    /// assert_eq!(data["x"]["y"], rivet_yaml::from_str::<Value>(r#"["z", "zz"]"#).unwrap());
    /// assert_eq!(data["x"]["y"][0], rivet_yaml::from_str::<Value>(r#""z""#).unwrap());
    ///
    /// assert_eq!(data["a"], rivet_yaml::from_str::<Value>(r#"null"#).unwrap()); // returns null for undefined values
    /// assert_eq!(data["a"]["b"], rivet_yaml::from_str::<Value>(r#"null"#).unwrap()); // does not panic
    /// # Ok(())
    /// # }
    /// ```
...
where
    I: Index,
{
    /// Write into a `rivet_yaml::Value` using the syntax `value[0] = ...` or
    /// `value["k"] = ...`.
    ///
    ...
    /// ```
    /// # fn main() -> rivet_yaml::Result<()> {
    /// let mut data: rivet_yaml::Value = rivet_yaml::from_str(r#"{x: 0}"#)?;
    ///
    /// // replace an existing key
    /// data["x"] = rivet_yaml::from_str(r#"1"#)?;
    ///
    /// // insert a new key
    /// data["y"] = rivet_yaml::from_str(r#"[false, false, false]"#)?;
    ///
    /// // replace a value in a sequence
    /// data["y"][0] = rivet_yaml::from_str(r#"true"#)?;
    ///
    /// // inserted a deeply nested key
    /// data["a"]["b"]["c"]["d"] = rivet_yaml::from_str(r#"true"#)?;
    ///
    /// println!("{:?}", data);
    /// # Ok(())
    /// # }
