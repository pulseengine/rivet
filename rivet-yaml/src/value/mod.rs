...
/// }
///
/// # fn try_main() -> Result<(), rivet_yaml::Error> {
/// let data = r#" { "level": 42 } "#;
/// let s: Settings = rivet_yaml::from_str(data)?;
///
...
/// A YAML sequence in which the elements are `rivet_yaml::Value`.
pub type Sequence = Vec<Value>;

/// Convert a `T` into `rivet_yaml::Value` which is an enum that can represent
/// any valid YAML data.
///
...
/// ```
/// # use rivet_yaml::Value;
/// let val = rivet_yaml::to_value("s").unwrap();
...
/// Interpret a `rivet_yaml::Value` as an instance of type `T`.
///
...
/// let s: String = rivet_yaml::from_value(val).unwrap();
/// assert_eq!("foo", s);
/// ```
...
impl Value {
    ...
    ///
    /// ```
    /// # fn main() -> rivet_yaml::Result<()> {
    /// use rivet_yaml::Value;
    ///
    /// let object: Value = rivet_yaml::from_str(r#"{ A: 65, B: 66, C: 67 }"#)?;
    /// let x = object.get("A").unwrap();
    /// assert_eq!(x, 65);
    ///
    /// let sequence: Value = rivet_yaml::from_str(r#"[ "A", "B", "C" ]"#)?;
    ...
    /// #
    /// # fn main() -> rivet_yaml::Result<()> {
    /// let object: Value = rivet_yaml::from_str(r#"
    /// A: [a, á, à]
    /// B: [b, b́]
    /// C: [c, ć, ć̣, ḉ]
    /// 42: true
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("null").unwrap();
    /// assert!(v.is_null());
    /// ```
    ///
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("false").unwrap();
    /// assert!(!v.is_null());
    /// ```
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("true").unwrap();
    /// assert!(v.is_bool());
    /// ```
    ///
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("42").unwrap();
    /// assert!(!v.is_bool());
    /// ```
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("5").unwrap();
    /// assert!(v.is_number());
    /// ```
    ///
    /// ```
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("1337").unwrap();
    /// assert!(v.is_i64());
    /// ```
    ///
    /// ```
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("256.01").unwrap();
    /// assert!(v.is_f64());
    /// ```
    ///
    /// ```
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("13.37").unwrap();
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("'lorem ipsum'").unwrap();
    /// assert!(v.is_string());
    /// ```
    ///
    /// ```
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("[1, 2, 3]").unwrap();
    ...
    ///
    /// ```
    /// # use rivet_yaml::{Value, Number};
    /// let v: Value = rivet_yaml::from_str("[1, 2]").unwrap();
    ...
    /// ```
    /// # use rivet_yaml::{Value, Number};
    /// let mut v: Value = rivet_yaml::from_str("[1]").unwrap();
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let mut v: Value = rivet_yaml::from_str("false").unwrap();
    ...
    /// ```
    /// # use rivet_yaml::Value;
    /// let v: Value = rivet_yaml::from_str("a: 42").unwrap();
    /// assert!(v.is_mapping());
    /// ```
    ///
    /// ```
    ...
    ///
    /// ```
    /// # use rivet_yaml::{Value, Mapping, Number};
    /// let v: Value = rivet_yaml::from_str("a: 42").unwrap();
    ///
    ...
    /// ```
    /// # use rivet_yaml::{Value, Mapping, Number};
    /// let mut v: Value = rivet_yaml::from_str("a: 42").unwrap();
    ...
    /// ";
    ///
    /// let mut value: Value = rivet_yaml::from_str(config).unwrap();
